...
def plot_info_on_video(key, ep):
  video = ep[key]
  video = plot_action_on_video(video, ep['action'])
  video = plot_value_on_video(video, {k: v for k, v in ep.items() if 'value' in k})
  return video
...
def plot_value_on_video(video, values):
  '''動画に行動の実数値のグラフをプロットする
  args:
    video: numpy array
      (sequence_length, w, h, c)
  ...
  for frame_idx in range(sequence_length):
    # 動画フレームと対応する行動データを取得
    value_data = list(values.values())[0][frame_idx]

    # 行動のグラフをプロット
    fig, ax = plt.subplots(figsize=(graph_width / 100, graph_height / 100), dpi=100)  # グラフのサイズを指定
    ax.text(0, 0, f'{value_data:.2e}', color='white', size='xx-small')
    ax.axis('off')  # 軸を非表示にする
    ...
    with BytesIO() as data:
      ...
      data.seek(0)
      graph_image = (plt.imread(data) * 255).astype(int)
      video_with_graph[frame_idx, :graph_height, :] = \
        np.where(
          graph_image[:, :, 3:4] == 0,
          video_with_graph[frame_idx, :graph_height, :],
          graph_image[:, :, :3]
