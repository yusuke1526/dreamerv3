...
def train_on_dataset(
  ...
  if args.task.split('_')[0] == "carla" and args.env.carla.auto_exploration:
    ...
    collect_agent = embodied.AutopilotAgent(train_env.act_space, train_env, **args)
  else:
    print('use RandomAgent for Prefill train dataset')
    collect_agent = embodied.RandomAgent(train_env.act_space)
  random_agent = embodied.RandomAgent(train_env.act_space)
  print('Prefill train dataset.')
  while len(train_replay) < max(args.batch_steps, args.train_fill):
    driver_train(collect_agent.policy, steps=100)
  ...
  if args.task.split('_')[0] == "carla" and args.env.carla.auto_exploration:
    policy_train = lambda *args: collect_agent.policy(
